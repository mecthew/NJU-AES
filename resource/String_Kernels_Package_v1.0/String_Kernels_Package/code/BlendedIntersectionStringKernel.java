/*
 This code computes the blended intersection string kernel.
...
public class BlendedIntersectionStringKernel
{
    private int ngramMinLength;
    private int ngramMaxLength;

    public BlendedIntersectionStringKernel(int ngramMinLength, int ngramMaxLength)
    {
        this.ngramMinLength = ngramMinLength;
        this.ngramMaxLength = ngramMaxLength;
    }

    public BlendedIntersectionStringKernel()
    {
        this(1, 5);
    }
    
    // Computes the blended intersection kernel among two strings.
    ...
    // Computes the blended intersection kernel matrix for a set of samples.
                ...
                for (int l = 0; l < sample.length(); l++)
                {
                    for (int d = ngramMinLength; d <= ngramMaxLength; d++)
                    {
                        if (l + d <= sample.length())
                        {
                            ...
                            {
                                K[i][j]++;
                                sampleNgrams.put(ngram, count - 1);
                            }
                        }
                    }
                }
    ...
    // Computes the blended intersection kernel matrix among two sets of samples.
